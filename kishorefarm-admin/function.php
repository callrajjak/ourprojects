...
function htmldecode($text) {
    return $cleantext = html_entity_decode($text);
}

function isValidUser() {
    if (trim($_SESSION['adminid']) != '' || trim($_SESSION['adminid']) != NULL) {
        return 1;
    } else {
        return 0;
    }
...
function clean($text) {
    ...
    return $cleantext = htmlentities($text, ENT_QUOTES, 'ISO-8859-1');
}

function cleanDB($text) {
    ...
    if ($lastpage > 1) {
        ...
        if ($lastpage < 7 + ($adjacents * 2)) {
            for ($counter = 1; $counter <= $lastpage; $counter++) {
                if ($counter == $page)
                    ...
                    $pagination.= "<li><a href='{$url}&apage=$counter'>$counter</a></li>";
            }
        }
        elseif ($lastpage > 5 + ($adjacents * 2)) {
            if ($page < 1 + ($adjacents * 2)) {
                ...
                $pagination.= "<li><a href='{$url}&apage=$lpm1'>$lpm1</a></li>";
                $pagination.= "<li><a href='{$url}&apage=$lastpage'>$lastpage</a></li>";
            }
            elseif ($lastpage - ($adjacents * 2) > $page && $page > ($adjacents * 2)) {
                $pagination.= "<li><a href='{$url}&apage=1'>1</a></li>";
                $pagination.= "<li><a href='{$url}&apage=2'>2</a></li>";
                $pagination.= "<li class='dot'>...</li>";
        ...
        if ($page < $counter - 1) {
            $pagination.= "<li><a href='{$url}&apage=$next'>Next >></a></li>";
            // $pagination.= "<li><a href='{$url}&apage=$lastpage'>Last</a></li>";
        } else {
    ...
    if (!empty($values)) {
        if (is_array($values)) {
            ...
            if ($count > 0) {
                $con_str .= " VALUES( ";
                for ($c = 0; $c < $count; $c++) {
                    $con_str .= "'" . striptags(cleanDB($values[$c])) . "', ";
                }
                $con_str = substr($con_str, 0, strlen($con_str) - 2);
                $con_str .= ") ";
            }
